...
export function contractAddEvent({ ownerPrincipal, eventUUID, priceInStx, endDateTime }: ContractAddEvent): Promise<any> {
...
export function contractUpdateEvent({ ownerPrincipal, eventUUID, priceInStx, endDateTime }: ContractAddEvent): Promise<any> {
    return new Promise((resolve, reject) => {
        const priceInNumber: number = Number(`${priceInStx}000`);
